...
<Update label="2026-09-15" description="A stored Recruiter password renews the session by itself; the contract chooser as tools">
  **Recruiter signs back in on its own.** Recruiter asks the seat holder for the LinkedIn password
  about every 30 days, and until now the only renewal was a person signing in through the cloud
  browser, on every seat, every month. The seat holder can now store the Recruiter password on the
  account, on the account's page in the app. When LinkedIn asks for it, in a scheduled sync or in the
  middle of a call, the platform signs the browser back in and the call goes on. The password is
  encrypted at rest, never returned by any read, used in one place, and forgotten the moment LinkedIn
  rejects it. No API call and no MCP tool takes or returns it; `recruiter_credentials_stored_at` on
  the account says whether one is stored. Where the platform could not renew the session by itself,
  `409 recruiter_reauth_required` now carries `context.sign_in_status` (why: `credentials_not_stored`,
  `wrong_credentials`, `challenge`, `sign_in_throttled`, `sign_in_failed`), `account_url` (where the
  seat holder stores the password) and a `suggestion` written for an agent to relay.
