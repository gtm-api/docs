...
answer with a `conflict` error. Read endpoints work as soon as data lands. When the first
sync finishes, the person who connected the account gets an email saying it is ready, with
what the sync brought in; a large account can run out of its first-day read budget part-way,
in which case the account carries `initial_sync_held_at`, the sync resumes on its own after
the daily reset, and the email arrives a day later and says so.

## 3. Check capabilities and limits

- `POST /api/linkedin-accounts/{sid}/check-premium-subscription` reports what the account's
  LinkedIn subscription actually supports (`has_premium`, `has_sn`, `recruiter_status`, and for a
  Recruiter seat its number and the session's expiry); `checks` narrows it to one step. Sales
  Navigator endpoints on an account without Sales Navigator fail upfront with
  `422 sales_nav_required`, Recruiter ones with `422 recruiter_required`, rather than mid-run.
