...
description: "Run contact enrichment on one LinkedIn profile: what comes back, when it runs on your own connected account versus our managed infrastructure, and how to read a partial result."
---
...
- A connected LinkedIn account for the free path, or the managed path for a profile
  nobody on the team is connected to.
...
| Where it can run | only an account of yours that is a 1st-degree connection | your pinned account, or our managed infrastructure |
...
## Your own account versus managed infrastructure
...
| `use_credits: true` | Runs on our managed infrastructure. `executed_on: "infra_pool"`, `reason: "infra_pool"` |
| Neither | `422 validation_failed`, with `field_errors.use_credits` starting `executor_required:`. Nothing runs, nothing is ledgered |
| Both | `422 validation_failed`. A pinned executor never delegates, so the pair is contradictory |
...
will not quietly move to a path you did not opt into.
</Warning>

### Contact info is the exception

`person-contact-info` requires a connected executor: LinkedIn reveals contact details to
1st-degree connections only, and a managed account would come back with an empty block.
...
| You passed `use_credits: true` | `422 validation_failed`: this method never runs on managed infrastructure, so omit the field |
...
| `served_from_cache: false` | Ran live: `executed_on: "own_account"` spent your account's daily budget, `executed_on: "infra_pool"` ran on managed infrastructure |
...
To run that call on managed infrastructure instead, re-issue with `use_credits: true` and
no `linkedin_account_sid`.
