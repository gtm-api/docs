...
| The account's browser could not serve the call this time | 503 | `browser_starting`, `browser_unavailable`, `plugin_unavailable`, `browser_maintenance`, `infrastructure_maintenance` | Nothing is wrong with the input. Wait for `retry_after` and send the call again: 30 seconds while a browser is still starting, one to two minutes after a lost session or a page that did not load, five to ten minutes while the browser or the fleet is being updated. A repeat under the same `idempotency_key` returns the stored failed row, so retry with a new key |
