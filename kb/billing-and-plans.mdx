...
<Warning>
A canceled workspace that cannot fall back to Sandbox (the owner already has a free
workspace, for example) has no coverage at all, and it is scheduled for deletion **30 days**
after the subscription ends. A paused or past-due subscription never schedules it. The
workspace owner gets an email with the date within a day, and a final one 3 days before
it. No workspace is deleted before that final email has gone out.

Resubscribe before the date and the deletion is canceled: the workspace comes back as it
was. On the date the workspace is deleted, and that cannot be undone. API keys are revoked,
members lose access at once, and the data is erased 30 days later.
</Warning>
