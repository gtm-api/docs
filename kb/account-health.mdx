...
description: "Every connected LinkedIn account carries a warmup score built from six compounding signals, a snapshot history, a limit-hit log and a block log. What each one records and what to do about it."
---
...
`check-target-block` runs the same two-browser check synchronously for a target you name. It costs **1
credit**, and only when the second-browser probe decides the outcome (`target_blocked` or
`target_unavailable`): a target already in the block log, a `skipped_recent` result inside the 24 hour
cache, or a probe that comes back as transport noise all cost **0 credits**. The first read spends **1
unit** of the account's `self_account_sync` budget, a limit and not a credit. `"force": true` bypasses
the cache.
...
`nickname`. A balance too short for the probe and a team with no second browser free are not errors:
both answer 200 with `outcome: "inconclusive"` and `credits.charged: 0`, nothing having run on a second
browser.
