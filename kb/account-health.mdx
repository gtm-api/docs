...
`status: "login_issue"`, or `"restricted"` when the platform checked the account right after
the logout from another LinkedIn account and found its public profile gone (a restriction, or
a closed account). There is no separate `logged_out` value.
</Warning>
...
|---|---|---|
| Login state | included `antidetect_browser.status` | Anything but `login_issue` or `restricted`. `login_issue` = signed out of LinkedIn, a re-login fixes it; `restricted` = signed out and the public profile is gone from LinkedIn, so the holder has to sort it out with LinkedIn first. `error_reason` and `last_logout_at` say what and when. NOT the account's own `status`, which is lifecycle and stays `active` through a logout |
...
number of matching senders whose browser sits in `login_issue` or `restricted`, so a summary that
skips the browser include still cannot miss them, and `counts.restricted_count` is the subset a
re-login alone will not bring back. `active_today_count` and `stale_count` are the heartbeat tallies.
...
| `linkedin-accounts.logged-out` | LinkedIn signed a sender out; `antidetect-browsers.logged-out` is the browser-level twin |
| `linkedin-accounts.restricted` | Right after the logout, the sender's public profile no longer answered from another LinkedIn account: a restricted (or closed) account. Follows `.logged-out`, never replaces it; `antidetect-browsers.restricted` is the browser-level twin |
