...
| `running_issue`, `start_issue`, `login_issue`, `client_error_investigation`, `support_error_investigation` | No. Their own handlers already powered the browser off, so start it yourself |
...
`login_issue`, `client_error_investigation`, `support_error_investigation` or `maintenance` waits for an explicit start, forever.
