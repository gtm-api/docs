...
soft-deleted workspace changes nothing: the 30-day cleanup period has to elapse and the purge has to
...
|---|---|---|---|
| Bad target: your own workspace; neither or both of `to_email` and `to_team_sid`; no workspace for that email | 422 | `validation_failed` | `field_errors.to_team_sid[0].rule = "cannot_share_to_self"`; `field_errors.to_email[0].rule = "exactly_one_of_to_email_or_to_team_sid"`; or `field_errors.to_email` with the unresolved address |
