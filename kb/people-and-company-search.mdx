...
description: "Run one live page of a LinkedIn, Sales Navigator or Recruiter people or company search: the pasted-URL and structured-filter input styles, how ids are resolved, what a row contains, and what a page costs."
---
...
[enrichment](/kb/enrichment) for those, a separate surface. The one exception is the Recruiter people search, whose
...
| `search-recruiter-people` | people | `page` 1 to 40, 25 rows a page; `url` XOR `filters` | Recruiter seat |
| `recruiter-param-id-lookup` | typeahead options | none, a fixed ten | Recruiter seat |
...
`employment_types`) take code arrays, listed in the [API reference](/api-reference/overview); note the Recruiter
...
A Recruiter URL carries no filters. After a search, Recruiter shows
`https://www.linkedin.com/talent/search?searchContextId=...&searchHistoryId=21502096450&searchKeyword=&start=0&uiOrigin=FACET_SEARCH`:
...
<Warning>
LinkedIn Recruiter throttles searches per seat. When a seat searches too fast, the call answers `429` with reason
`recruiter_search_usage_limit` and LinkedIn's own sentence: "You've reached a temporary usage limit. Wait a minute,
then retry your search." Wait the minute, then re-run the same page. This is LinkedIn's throttle, not your daily budget.
</Warning>
...
same rule as the [Recruiter inbox](/kb/inbox-and-message-sync).

## Resolving the ids that filters need

Three typeahead endpoints turn a human term into an id, over separate id spaces: an id from one never works on the
others. `recruiter-param-id-lookup` feeds the Recruiter search only: one `type` out of `occupation`, `skill`, `company`,
...
| Person field | People search | Sales Navigator people search | Recruiter people search |
|---|---|---|---|
| `ln_member_id`, `full_name`, `avatar_url` | filled | filled | filled |
| `ln_id` | filled | `null` | `null` |
| `sn_id` | `null` | filled | `null` |
| `nickname` (vanity slug) | filled | `null` | filled |
| `headline`, `position`, `company_name`, `location`, `connection_degree` | always `null` | always `null` | filled where LinkedIn shows them |

The Recruiter rows add `talent_id` (what `send-recruiter` addresses as `recipient_profile_id`), `industry`,
...
| omitted | | the service picks one of your connected accounts with remaining budget (Sales Navigator methods pick only accounts with a seat, Recruiter methods only seat holders with a live Recruiter session) |
...
<Warning>
Default budget: 100 scraping calls per day per connected account, paced about one call every 5 seconds. A short wait
for the slot is slept by the server, so a few typeaheads plus the search run one after another as slightly slower
calls, and firing them in parallel gains nothing. When the budget is spent, the account is on hold, or the wait for
the slot is longer than 20 seconds, the call returns `429 rate_limited` with `reason: "bucket_saturated"`, the `cause`
(`daily_saturation`, `held` or `delay_not_elapsed`) and a `retry_after` timestamp. Retry at that timestamp, not on a
fixed schedule and not earlier.
</Warning>
...
| The account's browser could not serve the search this time | 503 | `service_unavailable` | `context.reason` says why, always with `retry_after`: `browser_starting` (a cold start still warming up, 30 seconds), `browser_unavailable` (the session was lost and is being restarted, two minutes), `plugin_unavailable` (the search page did not load or did not answer, one minute), `browser_maintenance` or `infrastructure_maintenance` (the browser or the fleet is being updated, five to ten minutes). The run is marked `failed`; a repeat under the same `idempotency_key` returns that row, so retry with a new key |
