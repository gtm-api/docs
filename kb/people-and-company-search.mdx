...
| Field | Rule | Effect |
|---|---|---|
| `linkedin_account_sid` | `ln_ac_` plus 12 characters | runs on that account, `executed_on: "own_account"`, spends that account's daily `scraping` budget |
| `use_credits` | boolean | runs on our managed infrastructure, `executed_on: "infra_pool"`, `reason: "infra_pool"` |
| `idempotency_key` | string, max 128 characters | replay guard on (team, key) |

There is no silent fallback. Send exactly one executor (both, or neither, is a `422`): a pinned
...
account ramps lower still while it warms up. Runs on managed infrastructure do not touch your accounts' buckets at
all. Read the bucket
