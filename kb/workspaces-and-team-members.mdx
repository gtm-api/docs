...
`can_{verb}_{noun}`, plus an optional account slice. The catalog holds **43 tokens** (21
platform-level, 18 LinkedIn, 4 orchestration), and the wildcard `*` means all of them.

The picker offers three named presets over that list: **Superadmin** (everything, stored as `*`),
**Operator** (the default for a new invite: run outreach on every account, no billing, keys,
sessions or admin surfaces) and **View only** (read everything the product shows, act on nothing).
A preset is a starting point, not a stored role: after picking one you can still tick or untick
individual tokens. One token, `can_view_teams`, is part of every membership and cannot be unticked -
without it a member could not even resolve their own workspace.
...
list of `ln_ac_` sids is only those (**{n} of {total}**). An explicit empty list is refused with a
`422` - a member scoped to zero accounts would see a workspace that looks empty rather than
forbidden, which is a support ticket, not a grant.

<Note>
Both scoping surfaces are in the app. The invite dialog carries the permission editor, so an
invitation goes out with exactly the tokens you tick, and **Access** on a member row reopens the same
editor for somebody already in the workspace. Either one writes the full list, so the editor's state
is the grant: unticking is how you narrow. `PATCH /api/team-members/{sid}` does the same over the API.
A change applies immediately: the member's live tokens are invalidated and silently re-minted with
the new grant on their next request - no re-login, and no waiting for a token to expire. You can
only grant what you hold yourself: a request that exceeds your own access answers
`403 grant_exceeds_ceiling`.
</Note>
...
<Warning>
Deleting a workspace is owner-only, blocked while a paid plan is live, and cannot be undone. API keys
are revoked and members lose access the moment it is deleted, and its data is erased **30 days** later.
</Warning>
...
| A sid with the wrong prefix | `422 validation_failed`, `field_errors.sid[0].message` reads `Expected ts_mb_ prefix, got ...` | Member sids start with `ts_mb_` |
...
| A call with no `Team-SID` on a workspace-less token | `422 validation_failed`, with `field_errors.team_sid[0].rule` = `team_sid_required` | Get a token minted for a workspace. The header cannot supply one to a token that has none |
