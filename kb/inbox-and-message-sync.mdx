...
## Three inboxes, synced separately

The regular messenger, the Sales Navigator inbox and the LinkedIn Recruiter inbox are separate
surfaces with their own threads and their own clocks. Every thread carries a `messenger_type`.

| | Regular inbox | Sales Navigator | Recruiter |
|---|---|---|---|
| `messenger_type` | `linkedin` | `sales_navigator` | `recruiter` |
| Row on the **Account Sync** tab | **Conversations** | **Sales Nav Conversations** | **Recruiter Conversations** |
| Available when | always | only if the account has Sales Navigator | only if the account holds a Recruiter seat |
| Muting and marking read | supported | not supported, `is_muted` is always `null` | not supported |
| Verbs | `get-my-latest`, `sync-my-linkedin-conversations` | `get-my-latest-sales-nav`, `sync-my-sales-navigator-conversations` | `get-my-latest-recruiter`, `sync-my-recruiter-conversations` |

SN and Recruiter threads sync only once the platform detects the subscription (`premium_check`,
its own surface, every 360 minutes), which has to run before the **Sales Nav Conversations** or
**Recruiter Conversations** row appears. A basic-messenger verb on an SN or Recruiter thread
answers `422 messenger_type_mismatch`.

### The Recruiter inbox

The Recruiter inbox belongs to a seat, not to the profile. The premium check stamps the seat
number and its contract on the account (`recruiter_seat_id`, `recruiter_contract_id`), and every
Recruiter call needs them: without a seat the verbs answer `422 recruiter_required`, without the
number `422 recruiter_seat_unresolvable`. A candidate is identified by `talent_id` (the `AEMAA…`
id the Recruiter UI shows), which the platform decodes to the same `ln_member_id` as the
profile's `ln_id` and `sn_id`, so one candidate is one thread whichever id you send.

One sync run walks the INBOX tab and then UNRESOLVED: a thread you opened with an InMail sits
under UNRESOLVED until the candidate replies, and INBOX alone would never show it. ARCHIVED and
SCHEDULED are not walked. Pages carry 15 threads, the Recruiter client's own size, and there is
no realtime push for this inbox: a reply lands on the next sweep (120 minutes by default) or on a
head refresh.

LinkedIn keeps a separate Recruiter session of about 30 days, and the platform records its
expiry as `recruiter_session_expires_at`. Past that clock every Recruiter call answers
`409 recruiter_reauth_required` and the recurring run is cancelled with the same reason; the
**Recruiter Conversations** row and the Premium block on the account say so. The fix is a
sign-in to Recruiter in the account's own browser. The platform never stores the Recruiter
password.

Sending goes through `POST /api/linkedin-messages/send-recruiter`: a new thread by `talent_id`,
`ln_id` or `sn_id`, or a reply into an existing thread by its sid. LinkedIn allows **one InMail
per candidate per 24 hours** unless they reply sooner, so a second message inside that window
answers `429 rate_limited` with `reason: "recruiter_inmail_cooldown"` and a `retry_after`; a
reply the candidate has not answered yet is refused by LinkedIn under the same rule and surfaces
the same way. `get-my-recruiter-seat` and `get-my-hiring-projects` read the seat's own
entitlements and hiring projects.
...
invitations, the regular inbox, the premium check, the Sales Navigator and Recruiter inboxes when
the account holds those seats, then the profile snapshot) and the account unlocks when it finishes. Every thread is imported, since the run
...
During onboarding the `self_account_sync` daily budget is raised from **200 to 3000 calls per
...
| `recruiter_conversations` | 120 minutes | yes |
...
the browser plugin (the Sales Navigator sweep pages at 100, the Recruiter sweep at 15). Only threads whose event count or
...
`self_account_sync` budget: **200 calls per day**, paced about **one call a second**. When it
is spent, reads answer `429 rate_limited` with `limit_type:
...
7. **It was a push drop and the sweep has not run.** Your own reply typed on LinkedIn, a
   ...
   path: the wait is then up to 180 minutes (regular), 120 (Sales Navigator and Recruiter), 1440 (free plan).
...
A message carries `type` (`inbox` or `outbox`), `automation` (`auto`, `manual` or `synced`) and
`linkedin_type` (`message`, `inmail` or `connection_note`). A `connection_note` is the invitation
note: LinkedIn puts it into the thread when the invitation is accepted, so it arrives through the
sync like any other message (`automation: synced`) and carries that label. Messages are
...
<Warning>
A sync reset with `conversations`, `sales_navigator_conversations` or `recruiter_conversations`
selected **soft-deletes
...
A failed send is not retried automatically. Nothing picks it up on a timer, and the resend is something you or your team trigger.

The failure never lands in the conversation: a message row is only written on terminal success, so a failed attempt leaves no half-sent message.

If the failure was a timeout, the message may still have gone through on LinkedIn's side. Check the thread before resending, because a blind retry can double-send.
