...
| `activity_urn` | `get-post-commenters`, `get-post-reactors`, `get-post-resharers` |
...
Auto-scrapes never touch the credit pool: every page runs on the pinned executor account, so requests are stamped at zero credits.
A page spends that account's own `scraping` smart-limit bucket, one spend per page, and writes a row in [Data Requests](/kb/credits-and-data-requests).
