...
- A gtm-api workspace on any plan (the trial includes 5 account slots)
...
<Frame caption="The Senders page: every connected account with its browser, health and sync state.">
  <img src="/images/platform/senders-list.png" alt="Senders page listing LinkedIn accounts with statuses, browsers and health scores" />
</Frame>
...
<Frame caption="Cloud browser: choose the vendor and the proxy country; the summary names exactly what will be created.">
  <img src="/images/platform/connect-modal-cloud.png" alt="Connect LinkedIn account modal on the Cloud browser tab with vendor and proxy location fields" />
</Frame>
...
<Frame caption="Bring your own: the profile and its proxy remain yours; the platform only reads the session.">
  <img src="/images/platform/connect-modal-byo.png" alt="Connect LinkedIn account modal on the Existing profile ID tab with the GoLogin profile ID field" />
</Frame>
...
- `POST /api/linkedin-account-smart-limits/search` returns the account's current
  ...
  See [Smart limits and warmup](/platform/smart-limits-and-warmup).
...
- Check the proxy: a dead or blocked proxy prevents LinkedIn from loading. Run the
  ...
  ([how](/platform/antidetect-browsers-and-proxies#diagnose-a-proxy)).
