...
description: "Add the gtm-api MCP server to Claude, ChatGPT, Cursor, VS Code, Claude Code, Codex or any MCP-compatible runtime."
---
...
<CardGroup cols={2}>
  ...
  <Card title="VS Code" icon="code" href="vscode:mcp/install?%7B%22name%22%3A%22gtm-api%22%2C%22type%22%3A%22http%22%2C%22url%22%3A%22https%3A%2F%2Fmcp.gtm-api.com%2Fmcp%22%7D">
    One-click install (the button opens VS Code). Sign in on the consent page that follows.
  </Card>
...
```bash
codex mcp add gtm-api --url https://mcp.gtm-api.com/mcp
codex mcp login gtm-api
```

The first line registers the server in `~/.codex/config.toml`, the second runs the OAuth
flow and opens the consent page. No client id to pass: the server registers the client on
its own.
