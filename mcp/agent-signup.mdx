...
description: "How an AI agent registers its operator, connects itself, and handles the moment the trial ends."
---
...
<Steps>
  ...
  <Step title="Exchange the code">
    ...
    From here you hold the normal grant credential and installation token: connect to `https://mcp.gtm-api.com/mcp` or call REST directly. The operator lands in a 7-day trial workspace with 5 sender slots.
  </Step>
</Steps>
...
Write verbs start answering `402` with `subscription_required` once the trial lapses. That moment is handleable by the agent, not just reportable:

1. Read the state: search `billing-subscriptions` for the workspace and see `status` and `trial_ends_at`.
