...
- `field_errors` appears only on `validation_failed`, keyed by field. Each entry's `rule` is the machine reason (`ln_member_id_not_numeric`, `resend_not_available`) and its `message` the sentence for it; the same reason appears as `error.context.reason` on a `conflict`, `rate_limited` or `service_unavailable`. Every reason and its sentence is listed on the [Reasons](/concepts/reasons) page, generated from the backends' own catalogs.
