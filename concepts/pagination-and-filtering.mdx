...
- `page_size` ranges 0 to 500 and defaults to 50.
...
`sort` is an object of `field: direction` pairs, for example `{ "created_at": "desc" }`. Sortable fields are listed per endpoint.
